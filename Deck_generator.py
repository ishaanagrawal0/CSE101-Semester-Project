...
def randomDeck(allCards=allCards, winConditions=winConditions, spells=spells, airHitting=airHitting, buildings=buildings, support=support, champions=champions):
    deck = []
    cardImage = []
...
def optimalDeck():
    ...
    allCards = api.cardList(player_tag) #dictionary
    ...
    for i in range(len(cards)):
        for j in range(len(cards[i])):
            try:
                if allCards[cards[i][j]] >= cardLevelWeigths[i]:
                    newCards[i].append(cards[i][j])
            except:
                pass
    
    winConditions, spells, airHitting, buildings, support, champions = newCards[0], newCards[1], newCards[2], newCards[3], newCards[4], newCards[5]
    return randomDeck(list(allCards.keys()), winConditions, spells, airHitting, buildings, support, champions)
...
if __name__=="__main__":
    ...
    print(randomDeck(allCards, winConditions, spells, airHitting, buildings, support, champions))
    print(optimalDeck())